...
const list = computed(() => {
  let items = orders.pendingRequests
...
// RF-03: convertir solicitud en orden, asignando mecánico
const modalOpen = ref(false)
const selected = ref(null)
const mechanicId = ref('')
function openConvert(req) {
  selected.value = req
  mechanicId.value = auth.mechanics[0]?.id || ''
  ...
  <div class="card">
    <div class="card-body">
      <div class="flex-between" style="margin-bottom:16px;flex-wrap:wrap;gap:10px;">
        <h4 class="mt-0 mb-0">Solicitudes pendientes ({{ list.length }})</h4>
      ...
      <table>
        <thead><tr><th>Fecha</th><th>Cliente</th><th>Vehículo</th><th>Tipo</th><th>Fotos</th><th class="text-right">Acción</th></tr></thead>
        <tbody>
          <tr v-for="r in list" :key="r.id">
            ...
            <td>
              <div v-if="r.photos.length" class="photo-thumbs">
                <img v-for="(p,i) in r.photos" :key="i" :src="p.url" :title="p.name">
              </div>
              <span v-else class="text-muted">—</span>
            </td>
            <td class="text-right"><button class="btn btn-primary btn-sm" @click="openConvert(r)">Convertir en orden</button></td>
          </tr>
          <tr v-if="!list.length"><td colspan="6" style="text-align:center;color:var(--muted);padding:26px;">No hay solicitudes pendientes.</td></tr>
        </tbody>
      </table>
    </div>
  </div>

  <BaseModal v-if="modalOpen" title="Convertir en orden de trabajo" @close="modalOpen = false">
    ...
    <template #footer>
      <button class="btn btn-primary btn-block" @click="confirmConvert">Crear orden de trabajo</button>
    </template>
  </BaseModal>
</template>
...
.photo-thumbs img{ width:32px; height:32px; object-fit:cover; border-radius:6px; border:1px solid var(--border); }
